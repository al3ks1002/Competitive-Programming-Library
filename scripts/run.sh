...
usage() {
    ...
    printf "%bUsage -s source_file -t tests_directory [-c checker].%b\n" "$RED" "$NC"
    exit 1
}
...
check_not_empty() {
    ...
    file=$2
    if [[ -z $file ]]; then
        printf "%b$string argument is mandatory.%b\n" "$RED" "$NC"
        usage
        exit 1
    fi
}
...
check_if_exists() {
    ...
    file=$2
    if [[ ! -e $file ]]; then
        printf "%b$file: $string does not exist.%b\n" "$RED" "$NC"
        exit 1
    fi
}
...
check_if_directory() {
    ...
    dir=$2
    if [[ ! -d $dir ]]; then
        printf "%b$dir: $string is invalid.%b\n" "$RED" "$NC"
        exit 1
    fi
}
...
compile() {
    source=$1
    executable=$2
    if ! g++ -w -std=c++14 -O2 -o "$executable" "$source" ; then
        printf "%b$source: Compilation error.%b\n" "$RED" "$NC"
        exit 1
    fi
}
...
run_test() {
    ...
    if [[ $runtime_error != 0 ]]; then
        ...
        if [[ -z $checker ]]; then
            ...
            if ! dwdiff "$output" "$ok" > /dev/null; then
            ...
            if result=$(./checker_exec "$input" "$output" "$ok"); then
